    ...
    */
    ...
    fileprivate func _updateIlluminationMode(_ mode: CameraFlashMode) {
        if (cameraOutputMode != .stillImage) {
            ...
            _updateFlash(mode)
            captureSettings = getSettings(flashMode: flashMode)
        }
    }
    ...
    fileprivate func getSettings(flashMode: CameraFlashMode) -> AVCapturePhotoSettings {
        let settings = AVCapturePhotoSettings()
        for captureDevice in AVCaptureDevice.videoDevices {
            if captureDevice.hasFlash && cameraDevice == .back  {
                switch flashMode {
...
extension CameraManager : AVCapturePhotoCaptureDelegate {
    public func photoOutput(_ output: AVCapturePhotoOutput, didFinishProcessingPhoto photo: AVCapturePhoto, error: Error?) {
        ...
        if let dataImage = photo.fileDataRepresentation() {
            if burstModeEnabled {
                if self.capturedImages.count <= expectedPhotosCount {
                ...
                return self.imageCompletionBlock(CaptureResult(image))
            }
        }else {
            if !burstModeEnabled {
                return self.imageCompletionBlock(.failure(CaptureError.noImageData))
            }
        }
    }
