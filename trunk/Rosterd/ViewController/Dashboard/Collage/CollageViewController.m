...
//    _moreButton = [[UIBarButtonItem alloc] initWithImage:[UIImage imageNamed:@"ic_DeleteCard"]
...
-(void) viewWillAppear:(BOOL)animated{
    ...
    selectedPhotoCount =[_collage.selectedPhotos count];
//    if ([_collage.collagePhotos count]== 0) {
//        [_ViewDone setHidden:YES];
//    } else {
//        [_ViewDone setHidden:NO];
//    }
}
...
- (IBAction)btnDoneClick {
    ...
    if(_collage.selectedPhotos.count > 0) {
        [self.delegate imagesData:[self makeImage]];
        [self.navigationController popViewControllerAnimated:YES];
    } else {
        [self.navigationController popViewControllerAnimated:YES];
    }
...
-(void)showActionSheet{
    ...
    UIAlertController *actionSheet = [UIAlertController alertControllerWithTitle:@"Select image from" message:@"" preferredStyle:UIAlertControllerStyleActionSheet];
